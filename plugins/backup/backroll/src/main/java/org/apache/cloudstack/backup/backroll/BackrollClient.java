...
public class BackrollClient {
                ...
                if(response.state.equals("SUCCESS")) {
                    if(response.info.archives.size() > 0) {
                        for (BackupInfos infos : response.info.archives) {
                            backups.add(new BackrollVmBackup(infos.id, infos.name, infos.start));
                        }
                    }
                }
            }
    ...
    private boolean isResponseUnauthorized(final HttpResponse response) {
        return response.getStatusLine().getStatusCode() == HttpStatus.SC_UNAUTHORIZED ? true : false;
    }
    ...
    private boolean isAuthentificated() {
        ...
        try {
            ...
            result = !isResponseUnauthorized(response);
            EntityUtils.consumeQuietly(response.getEntity());
        } catch (IOException e) {
